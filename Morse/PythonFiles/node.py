...
class Node():
    ...
    """
    def agent_go(self, wind_rose_action):
        while not rospy.is_shutdown():
            ...
            if (x_min < self.pose.pose.position.x < x_max) and (y_min < self.pose.pose.position.y < y_max): 
                print y_min, y_max, x_min, x_max, grid_1, state #"Im in a grid and my state is %s and grid %s" % (state, grid)
                """
                ...
                final_choice = self.wind_rose_function(self.wind_rose, wind_rose_action) #second attribute: 1 - Want to go North, 2 - Want to go East, 3 - Want to go South, 4 - Want to go West #choice is the Q learning option
                ...
                print("Movement chose %d" %final_choice)
            else:
...
if __name__=='__main__':   
    ...
    grid = GridData()
    sensor_node.agent_go(3) #1 - North, 2 - East, 3 - South, 4 - West
